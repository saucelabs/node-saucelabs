...
SauceLabs.prototype.createPublicLink = function (id, date, useHour, callback) {
  ...
  var link = generateLink(this.options.hostname, this.options.username + ':' + this.options.password, date, id);
  callback(null, link);
};
...
function makeRequest(options, body, callback) {
  request(options, function (error, response, body) {
    ...
    if(error){
      callback('Could not send request: ' + error.message);
    }
