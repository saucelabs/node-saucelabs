...
for (const [commandName, options] of PROTOCOL_MAP) {
  ...
  const example = `api.${commandName}(${inlineParameters}${separator}${
    commandOptions.length ? '{ ...options }' : ''
  })`;

  docs += '    <tr>\n';
  docs += '      <td>\n';
  docs += `        <b>${options.method.toUpperCase()}</b> <code>${
    options.endpoint
  }</code><br>\n`;
  docs += `        ${
...
fs.writeFileSync(path.join(__dirname, '..', 'docs', 'interface.md'), docs, {
  encoding: 'utf-8',
});
